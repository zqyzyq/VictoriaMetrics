...
func TestPackedTimeseries_Unpack(t *testing.T) {
	...
	f := func(name string, dataBlocks []blockData, updateBlocks []blockData, wantResult *Result) {
		t.Run(name, func(t *testing.T) {
			...
			for _, dataBlock := range dataBlocks {
				...
				addr, err := tbf.WriteBlockData(storage.MarshalBlock(nil, bb))
				if err != nil {
			...
			if err := pts.Unpack(&dst, &tbf, tr); err != nil {
	...
	f("2 blocks at the border of time range",
		...
		&Result{
			MetricName: mn,
			Values:     []float64{decimal.StaleNaN, 15, 2, 3, 4, 5, 6},
			Timestamps: []int64{0, 10, 15, 30, 35, 40, 45},
		})
